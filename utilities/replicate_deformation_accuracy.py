...
for i in tqdm(range(len(key_ages) -1)):
    ...
    older_points_heidi = points_dataset_h[[col_format.format(name_older_age, 'x','sagittal'),
                                col_format.format(name_older_age,  'y', 'horizontal'),
                                col_format.format( name_older_age, 'z', 'coronal')]].values
    younger_points_heidi = points_dataset_h[[col_format.format(name_younger_age, 'x','sagittal'),
                                col_format.format(name_younger_age, 'y', 'horizontal'),
                                col_format.format(name_younger_age, 'z', 'coronal')]].values
    older_points_ingvild = points_dataset_i[[col_format.format(name_older_age, 'x','sagittal'),
                                col_format.format(name_older_age, 'y', 'horizontal'),
                                col_format.format(name_older_age, 'z', 'coronal')]].values
    younger_points_ingvild = points_dataset_i[[col_format.format(name_younger_age, 'x','sagittal'),
                                col_format.format(name_younger_age, 'y', 'horizontal'),
                                col_format.format(name_younger_age, 'z', 'coronal')]].values
    if mean_older_points is None:
        mean_older_points = np.mean([older_points_ingvild, older_points_heidi], axis=0)
    points = CCF_translator.pointset(values=mean_older_points,
                    space=target_space,
                    age_PND=older_age,
                    voxel_size_um=20)
    ...
    i2h_collect.append(i2h)
    h2d_collect.append(h2d)
    i2d_collect.append(i2d)
    d2a_collect.append(d2a)
