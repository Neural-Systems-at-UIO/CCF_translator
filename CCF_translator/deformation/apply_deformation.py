...
def extract_metadata(metadata, source_metadata, target_metadata, start, stop):
    translation_metadata = metadata[(source_metadata == stop) & (target_metadata == start)]
...
def load_and_combine_deformation(deform_arr, deform_path, vector, translation_metadata, old_voxel_size, final_voxel_size, target_shape):
...
def combine_route(route, original_voxel_size, base_path, metadata):
    ...
    source_metadata = metadata["source_space"] + "_P" + metadata["source_age_pnd"].astype(str)
    target_metadata = metadata["target_space"] + "_P" + metadata["target_age_pnd"].astype(str)
    temp_padding = None

    for i in range(1, len(route)):
        start = route[i - 1]
        stop = route[i]
        translation_metadata = extract_metadata(metadata, source_metadata, target_metadata, start, stop)

        if translation_metadata['padding_micron'][0] != "[[0, 0], [0, 0], [0, 0]]":
            deform_arr, temp_padding = handle_padding(deform_arr, translation_metadata['padding_micron'][0], original_voxel_size)
            pad_sum += temp_padding

        if translation_metadata['dim_order'][0] != '[0, 1, 2]':
            deform_arr, target_shape, pad_sum, temp_padding, dim_order_sum = handle_dim_order(
                deform_arr, translation_metadata['dim_order'][0], target_shape, pad_sum, temp_padding, dim_order_sum)

        if translation_metadata['dim_flip'][0] != '[False, False, False]':
            deform_arr, pad_sum, flip_sum = handle_dim_flip(deform_arr, translation_metadata['dim_flip'][0], pad_sum, flip_sum)

        if translation_metadata["file_name"][0] != 'False':
            vector = int(translation_metadata["vector"][0])
            deform_path = os.path.join(base_path, "metadata", "deformation_fields", translation_metadata["source_space"][0], translation_metadata["file_name"][0])
            ...
            deform_arr, final_voxel_size, target_shape, old_voxel_size = load_and_combine_deformation(
                deform_arr, deform_path, vector, translation_metadata,  old_voxel_size, final_voxel_size, target_shape)
